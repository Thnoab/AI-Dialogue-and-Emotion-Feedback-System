...
class Memorize:
    ...
    def communicate(self, session_id, character_id, msg, abr):
        print("communicate开始, character_id =", character_id)
        self.cor.execute(
            '''
            INSERT INTO memory(session_id, character_id, mode, submit, summary)
            VALUES (?, ?, ?, ?, ?)
            ''',
            (session_id, character_id, "single_daily", msg, abr)
        )
        self.conn.commit()
    ...
    def request(self, session_id, character_id, msg, abr1, ret, abr2, emt, his):
        print("request开始, character_id =", character_id)
        row_id = self.communicate(session_id, character_id, msg, abr1)
    ...
    def quote(self, session_id, character_id):
        ...
        self.cor.execute(
            ...
            LIMIT 1
            ''',
            (session_id, character_id, "single_daily")
        )
    ...
    def rappelez(self, session_id, character_id, n):
        ...
        if n > 0:
            self.cor.execute(
                ...
                LIMIT ?
                ''',
                (session_id, character_id, mode, n)
            )
        ...
        if recent_ids:
            ...
            self.cor.execute(
                ...
                ORDER BY id ASC
                ''',
                [session_id, character_id, mode] + recent_ids
            )
        ...
        if m > 0:
            if recent_ids:
                placeholders = ",".join(["?"] * len(recent_ids))
                params = [session_id, character_id, mode] + recent_ids + [m]
                self.cor.execute(
                    f'''
                ...
                self.cor.execute(
                    ...
                    LIMIT ?
                    ''',
                    (session_id, character_id, mode, m)
                )
...
# ==================== 多人模式 ====================
# =================================================
    def communicate_multi(self, session_id, character_id, msg, abr):
        self.cor.execute(
            '''
            INSERT INTO memory(session_id, character_id, mode, submit, summary)
            VALUES (?, ?, ?, ?, ?)
            ''',
            (session_id, character_id, "group_tavern", msg, abr)
        )
        self.conn.commit()
        return self.cor.lastrowid
    
    def request_multi(self, session_id, character_id, msg, abr1, ret, abr2, emt, his):
        row_id = self.communicate_multi(session_id, character_id, msg, abr1)
        self.reply(ret, abr2, emt, row_id)
        self.calcu_result(his, row_id)

    def quote_multi(self, session_id, character_id):
        self.cor.execute(
            '''
            SELECT history
            FROM memory
            ...
            LIMIT 1
            ''',
            (session_id, character_id, "group_tavern")
        )
        row = self.cor.fetchone()
        if row is None:
            return None
    ...
    def rappellent(self, session_id, character_id, n):
...
# =================================================
# DS8 新增，多人跑团模式
    def rappeler(self, session_id, character_id, current_keywords=None, fixed_memory="", n=25, total=65, relevant_limit=5):
        """
        跑团模式专属记忆算法。
    ...
    def anaklisi(self, session_id, character_id, keywords, limit=5):
        """
        跑团模式相关旧记忆召回。
    ...
    def request_trpg(self, session_id, character_id, submit, reply, summary, motto, emotion, history, keywords):
        """
        跑团模式写入数据库。
        ...
        self.cor.execute(
            '''
            INSERT INTO memory
            (session_id, character_id, mode, submit, reply, summary, motto, emotion, history, keywords)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ''',
            (
                session_id,
    ...
    def quote_trpg(self, session_id, character_id):
        """
        ...
        self.cor.execute(
            ...
            LIMIT 1
            ''',
            (session_id, character_id, "group_trpg")
        )
        row = self.cor.fetchone()
...
# =================================================
# DS11 新增，所有模式清空记录
    def obliviscere(self, session_id, mode):
        """
