...
from DS2_Memo import Memorize  # 新增
import json  # 新增
...
class ChatRequest(BaseModel):
    message: str
    system_prompt: Optional[str] = "你必须严格输出JSON格式,你是一个DeepSeek助手，回答要简洁有用。"
...
def chat_with_deepseek(req: ChatRequest):
    ...
    2. abr1：对用户输入的简要概括
    3. abr2：对AI回复的简要概括
    4. emotion：绝对值为1的三种情绪向量 [glee,buzz,stance]
    针对emotion这里，情绪向量的绝对值为1。如果这一项不存在，则为0，如果为正向，则为1；反之如果为负向的，则为-1。
    emotion：一个长度为3的情绪向量 [glee, buzz, stance]，
    每一项取值范围为 -1 到 1。
    如果该情绪不存在，则为 0。
    细致一些，遵循以下：
    ...
    try:
        ...
        print("准备写入数据库")
        memo.request(req.message,abr1,reply,abr2,emotion)
        print("数据库写入完成")
        # ----------------------------------------------------------
